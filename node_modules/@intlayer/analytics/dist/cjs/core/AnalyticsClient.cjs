...
const require_utils_idle = require('../utils/idle.cjs');
const require_core_abTesting = require('./abTesting.cjs');
const require_core_eventBuffer = require('./eventBuffer.cjs');
const require_core_sampling = require('./sampling.cjs');
const require_core_session = require('./session.cjs');
const require_core_transport = require('./transport.cjs');
...
const createAnalyticsClient = (config) => {
	const sessionId = require_core_session.getSessionId();
	const sampledIn = require_core_sampling.isSampledIn(sessionId, config.sampleRate);
	const buffer = require_core_eventBuffer.createEventBuffer({ max: config.maxBufferSize });
	...
	const flush = (useBeacon = false) => {
		...
		for (let start = 0; start < events.length; start += 200) require_core_transport.sendEvents(endpoint, {
	...
	const getVariant = (experimentKey, variants, weights) => require_core_abTesting.assignVariant(sessionId, experimentKey, variants, weights);
	...
	const start = () => {
		...
		flushTimer?.unref?.();
		require_utils_idle.onIdle(attachListeners);
	};
	const stop = () => {
		if (!started) return;
		started = false;
...
exports.MAX_EVENTS_PER_SEND = MAX_EVENTS_PER_SEND;
