	...
	const optionalJoinBaseDir = (pathInput) => {
		...
		try {
			...
			try {
				...
				if (!pathInput.startsWith(".") && !isAbsolute(pathInput)) absolutePath = requireFunction.resolve(`${pathInput}/package.json`, { paths: [projectBaseDir] });
				else throw err;
			}
		} catch {
	...
	const optionalJoinBaseDir = (pathInput) => {
		...
		try {
			...
			try {
				...
				if (!pathInput.startsWith(".") && !isAbsolute(pathInput)) absolutePath = requireFunction.resolve(`${pathInput}/package.json`, { paths: [systemConfig.baseDir] });
				else throw err;
			}
		} catch {
			try {
				try {
					absolutePath = __require.resolve(pathInput, { paths: [systemConfig.baseDir] });
				} catch (err) {
					if (!pathInput.startsWith(".") && !isAbsolute(pathInput)) absolutePath = __require.resolve(`${pathInput}/package.json`, { paths: [systemConfig.baseDir] });
					else throw err;
				}
			} catch {
				absolutePath = isAbsolute(pathInput) ? pathInput : join(systemConfig.baseDir, pathInput);
			}
		}
		try {
			if (statSync(absolutePath).isFile()) return dirname(absolutePath);
...
//#endregion
export { buildAnalyticsFields, buildBrowserConfiguration, buildConfigurationFields, buildEditorFields, buildInternationalizationFields, buildLogFields, buildRoutingFields, extractBrowserConfiguration };
