...
const require_utils_babelParser = require('../utils/babelParser.cjs');
...
const nodeTypeStrategies = {
	...
	[_intlayer_types_nodeType.SELECT]: {
		kind: "helperRecord",
		helper: "select",
		getExtraArguments: (node) => typeof node.variable === "string" && node.variable.length > 0 ? [b.literal(node.variable)] : []
	},
	...
	[_intlayer_types_nodeType.TEXT]: { kind: "unwrap" },
	[_intlayer_types_nodeType.NUMBER]: { kind: "unwrap" },
	[_intlayer_types_nodeType.BOOLEAN]: { kind: "unwrap" },
	[_intlayer_types_nodeType.NULL]: { kind: "unwrap" },
	[_intlayer_types_nodeType.REACT_NODE]: { kind: "preserve" },
	[_intlayer_types_nodeType.PREACT_NODE]: { kind: "preserve" },
	[_intlayer_types_nodeType.SOLID_NODE]: { kind: "preserve" },
	[_intlayer_types_nodeType.OBJECT]: { kind: "structural" },
	[_intlayer_types_nodeType.ARRAY]: { kind: "structural" },
	[_intlayer_types_nodeType.UNKNOWN]: { kind: "structural" }
};
/** Every `intlayer` helper the transform is allowed to rewrite. */
const helperNames = new Set(Object.values(nodeTypeStrategies).flatMap((strategy) => "helper" in strategy ? [strategy.helper] : []));
/** Helpers wrapping a single content, e.g. `md(t({ … }))`. */
const wrapperHelperNames = new Set(Object.values(nodeTypeStrategies).flatMap((strategy) => strategy.kind === "helperValue" ? [strategy.helper] : []));
/**
...
const isMultilingualNode = (val) => {
	...
	if (strategy.kind === "helperValue" || strategy.kind === "unwrap") return isMultilingualNode(nodeData);
	if (strategy.kind === "helperRecord" && nodeData && typeof nodeData === "object") return Object.values(nodeData).some((variant) => isMultilingualNode(variant));
	return false;
};
/**
* Checks whether two AST nodes are literals holding the same value.
*/
const isEquivalentLiteral = (left, right) => {
	const isLiteral = (node) => n.Literal.check(node) || n.StringLiteral.check(node) || n.NumericLiteral.check(node) || n.BooleanLiteral.check(node);
...
*/
const syncCallArguments = (callNode, desiredArguments) => {
	if (!(callNode.arguments.length === desiredArguments.length && desiredArguments.every((desired, index) => {
...
const buildNodeForValue = (val, existingNode, fallbackLocale, requiredImports) => {
	...
	if (fallbackLocale && existingNode && !isMultilingualNode(val)) {
		...
		if ((!val || typeof val !== "object") && n.CallExpression.check(existingNode) && n.Identifier.check(existingNode.callee) && wrapperHelperNames.has(existingNode.callee.name)) {
	...
	if (typeof val === "string" || typeof val === "number" || typeof val === "boolean") {
		if (unwrappedExisting) {
			if (n.TemplateLiteral.check(unwrappedExisting) && unwrappedExisting.quasis[0] && unwrappedExisting.expressions.length === 0) {
	...
	if (Array.isArray(val)) {
		if (unwrappedExisting && n.ArrayExpression.check(unwrappedExisting)) {
			...
			val.forEach((item, i) => {
				const elementNode = buildNodeForValue(item, elements[i], fallbackLocale, requiredImports);
				elements[i] = elementNode === SKIP_WRITE ? elements[i] ?? b.literal(null) : elementNode;
			});
			if (elements.length > val.length) elements.length = val.length;
			unwrappedExisting.elements = elements;
			return existingNode;
		} else return b.arrayExpression(val.map((item) => {
			const elementNode = buildNodeForValue(item, null, fallbackLocale, requiredImports);
			return elementNode === SKIP_WRITE ? b.literal(null) : elementNode;
		}));
	}
	const nodeType = val && typeof val === "object" && !Array.isArray(val) ? (0, _intlayer_core_dictionaryManipulator.getNodeType)(val) : null;
	if (nodeType) {
		const strategy = nodeTypeStrategies[nodeType];
		if (strategy.kind === "preserve") return existingNode ?? SKIP_WRITE;
		const hasNodeData = Object.hasOwn(val, nodeType);
		const nodeData = val[nodeType];
		if (strategy.kind !== "structural" && hasNodeData) {
			if (strategy.kind === "unwrap") return buildNodeForValue(nodeData, existingNode, fallbackLocale, requiredImports);
			const { helper } = strategy;
			requiredImports.add(helper);
			const isMatchingCall = existingNode && n.CallExpression.check(existingNode) && n.Identifier.check(existingNode.callee) && existingNode.callee.name === helper;
			if (strategy.kind === "helperRecord") {
				const recordArgument = isMatchingCall && existingNode.arguments.length > 0 && n.ObjectExpression.check(existingNode.arguments[0]) ? existingNode.arguments[0] : b.objectExpression([]);
				updateObjectLiteral(recordArgument, nodeData ?? {}, fallbackLocale, requiredImports);
				const callArguments = [recordArgument, ...strategy.getExtraArguments?.(val) ?? []];
				if (isMatchingCall) {
			...
			if (strategy.kind === "helperValue") {
				const contentArgument = buildNodeForValue(nodeData, isMatchingCall && existingNode.arguments.length > 0 ? existingNode.arguments[0] : null, fallbackLocale, requiredImports);
				if (contentArgument === SKIP_WRITE) return existingNode ?? SKIP_WRITE;
				if (isMatchingCall) {
					syncCallArguments(existingNode, [contentArgument, ...existingNode.arguments.slice(1)]);
					return existingNode;
				}
...
const updateObjectLiteral = (node, data, fallbackLocale, requiredImports) => {
	for (const [key, val] of Object.entries(data)) {
		...
		if (existingProp) {
			const valueNode = buildNodeForValue(val, existingProp.value, fallbackLocale, requiredImports);
			if (valueNode !== SKIP_WRITE) existingProp.value = valueNode;
		} else {
...
const transformJSFile = async (fileContent, dictionary, fallbackLocale, noMetadata) => {
	...
	let ast;
	try {
		ast = recast.parse(fileContent, { parser: require_utils_babelParser.babelTsParser });
	} catch (error) {
		console.error({ error });
		return fileContent;
	}
