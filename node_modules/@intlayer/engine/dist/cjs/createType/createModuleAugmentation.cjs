...
const generateTypeIndexContent = (typeFiles, configuration, zodToTsFns) => {
	...
	const { locales, requiredLocales, strictMode, defaultLocale } = internationalization;
	let fileContent = "import \"intlayer\";\n";
	const dictionariesRef = typeFiles.map((dictionaryPath) => ({
		relativePath: `./${(0, _intlayer_config_utils.normalizePath)((0, node_path.relative)(moduleAugmentationDir, dictionaryPath))}`,
...
const createModuleAugmentation = async (configuration) => {
	...
	let zodToTsFns = null;
	if (Object.keys(configuration.schemas ?? {}).length > 0) try {
	...
	const tsFilePath = (0, node_path.join)(moduleAugmentationDir, "intlayer.d.ts");
	await require_writeFileIfChanged.writeFileIfChanged(tsFilePath, tsContent);
};
