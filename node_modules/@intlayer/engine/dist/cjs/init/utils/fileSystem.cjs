...
const detectNextIntlMessagesPattern = async (rootDir) => {
	for (const requestFile of NEXT_INTL_REQUEST_FILES) {
		...
		const requestDir = (0, node_path.dirname)((0, node_path.resolve)(rootDir, requestFile));
		const absoluteTemplate = (0, node_path.resolve)(requestDir, importPath);
		const relativeTemplate = (0, node_path.relative)(rootDir, absoluteTemplate).split(node_path.sep).join("/");
