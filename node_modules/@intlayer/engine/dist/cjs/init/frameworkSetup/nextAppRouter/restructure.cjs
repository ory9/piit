...
const rewriteRelativeImports = (code, oldAbs, newAbs, context) => {
	const ast = recast.parse(code, { parser: require_utils_babelParser.babelTsParser });
	let changed = false;
	const rewriteSource = (sourceNode) => {
		...
		const oldTargetAbs = (0, node_path.resolve)((0, node_path.dirname)(oldAbs), specifier);
...
const restructureAppIntoLocale = async (rootDir, appDir) => {
	...
	for (const newAbs of movedFiles) {
		...
		const oldAbs = (0, node_path.join)(appDirAbs, relFromLocale);
