...
const getCallerDir = () => {
	...
	try {
		...
		for (const frame of frames) {
			const raw = typeof frame.getFileName === "function" ? frame.getFileName() : null;
...
const readAsset = (relPath, encoding = "utf8") => {
	...
	*/
	const callerSubpath = (0, node_path.relative)(distRoot, callerDirAbs).split("\\").join("/").replace(/^(?:dist\/)?(?:esm|cjs)\//, "").replace(/^_virtual\//, "");
