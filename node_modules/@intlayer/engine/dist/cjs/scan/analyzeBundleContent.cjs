...
*/
const allLocaleValues = new Set(Object.values(_intlayer_types_allLocales.ALL_LOCALES));
...
const analyzeBundleContent = (chunks, htmlContent, currentLocale, totalPageSize) => {
	...
	const pageStrings = new Set(require_scan_parseHtml.extractVisibleTextStrings(htmlContent));
	let renderedContentSize = 0;
	pageStrings.forEach((text) => {
		renderedContentSize += require_scan_parseHtml.byteLength(text);
	});
	...
	for (const chunk of chunks) {
		...
		const analysis = {
			url: chunk.url,
			fileSize: require_scan_parseHtml.byteLength(chunk.content),
			totalLocaleSize,
			unusedLocaleSize,
			usedLocaleSize,
			dictionariesFound,
...
//#endregion
exports.analyzeBundleContent = analyzeBundleContent;
//# sourceMappingURL=analyzeBundleContent.cjs.map