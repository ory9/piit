...
var DictionariesLogger = class {
	...
	*/
	setPluginError(error, pluginName) {
		if (this.isFinished) return;
		const errorMessage = extractErrorMessage(error);
		this.pluginError = pluginName ? `${pluginName}: ${errorMessage}` : errorMessage;
		this.render();
	}
	render() {
		...
		if (!(this.expectRemote && this.remoteCheckInProgress && remoteTotal === 0)) {
			if (isLocalDone) lines.push(`${this.prefix} ${v} Local content: ${colorize(`${localDone}`, ANSIColors.GREEN)}${colorize(`/${localTotal}`, ANSIColors.GREY)}`);
		...
		if (remoteTotal > 0 || this.remoteCheckInProgress || this.remoteError) {
			if (this.remoteError) lines.push(`${this.prefix} ${x} Remote content: ${colorize(this.remoteError, ANSIColors.RED)}`);
		...
		if (pluginTotal > 0 || this.pluginError) {
			const pluginNamesSuffix = this.pluginNames.length > 0 ? colorize(` (${this.pluginNames.join(", ")})`, ANSIColors.GREY) : "";
			if (this.pluginError) lines.push(`${this.prefix} ${x} Plugin content: ${colorize(this.pluginError, ANSIColors.RED)}`);
			else if (isPluginDone) lines.push(`${this.prefix} ${v} Plugin content: ${colorize(`${pluginDone}`, ANSIColors.GREEN)}${colorize(`/${pluginTotal}`, ANSIColors.GREY)}${pluginNamesSuffix}`);
			else lines.push(`${this.prefix} ${clock} Plugin content: ${colorize(`${pluginDone}`, ANSIColors.BLUE)}${colorize(`/${pluginTotal}`, ANSIColors.GREY)}${pluginNamesSuffix}`);
		}
