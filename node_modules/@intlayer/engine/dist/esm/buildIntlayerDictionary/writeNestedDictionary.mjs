...
*/
const getNestedDictionaryPath = (dictionariesDir, key, extension = "mjs") => resolve(dictionariesDir, NESTED_DICTIONARIES_SUBDIR, `${key}.${extension}`);
/**
...
const generateNestedDictionaryEntryPoint = (key, nestedKeys, format = "esm") => {
	...
	const identifierOf = (nestedKey) => `_${getPathHash(nestedKey)}`;
...
*/
const writeNestedDictionaries = async (nestedDictionaryGraph, configuration, formats = OUTPUT_FORMAT) => {
	...
	await parallelize([...nestedDictionaryGraph.entries()].sort(([a], [b]) => a.localeCompare(b)), async ([key, nestedKeys]) => {
		await parallelize(formats, async (format) => {
			...
			await writeFileIfChanged(entryPath, content).catch((error) => {
				console.error(`Error creating nested ${colorizePath(entryPath)}:`, error);
			});
		});
	});
};
...
//#endregion
export { NESTED_DICTIONARIES_SUBDIR, generateNestedDictionaryEntryPoint, getNestedDictionaryPath, writeNestedDictionaries };
//# sourceMappingURL=writeNestedDictionary.mjs.map