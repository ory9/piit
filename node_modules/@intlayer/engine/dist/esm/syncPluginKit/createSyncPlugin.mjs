	...
	*/
	...
	const loadDictionaries = async ({ configuration }) => {
		...
		if (entries.length === 0) {
			...
			appLogger(`[${name}] No dictionaries found at locations matching source pattern: ${colorizePath(sourceDescription)}`, { level: "warn" });
		}
		...
		for (const entry of entries) {
			...
			const relativeFilePath = entry.filePath ? relative(baseDir, entry.filePath) : void 0;
	...
	const formatOutput = async ({ dictionary, configuration }) => {
		...
		const { baseDir } = configuration.system;
		if (resolve(baseDir, canonicalUri) !== resolve(baseDir, dictionary.filePath)) return dictionary;
		const { formatDictionaryOutput } = await import("../formatDictionary.mjs");
	...
	const afterBuild = async ({ dictionaries, configuration }) => {
		...
		const { formatDictionaryOutput } = await import("../formatDictionary.mjs");
		const { parallelize } = await import("../utils/parallelize.mjs");
		const context = { configuration };
...
//# sourceMappingURL=createSyncPlugin.mjs.map