...
*/
const STRUCTURAL_MISMATCH_PENALTY = -5;
...
const alignBaseAndTargetBlocks = (baseBlocks, targetBlocks) => {
	...
	while (i > 0 || j > 0) if (i > 0 && j > 0 && traceMatrix[i][j] === "diagonal") {
		...
		const similarityScore = computeJaccardSimilarity(baseBlocks[baseIndex].anchorText, targetBlocks[targetIndex].anchorText, 3);
		result.unshift({
			baseIndex,
			targetIndex,
			similarityScore
...
//#endregion
export { alignBaseAndTargetBlocks };
