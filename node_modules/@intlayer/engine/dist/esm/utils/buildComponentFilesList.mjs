...
const buildComponentFilesList = (config, excludePattern) => {
	...
	const patterns = [...traversePattern, ...normalizeToArray(compilerTransformPattern)].filter((pattern) => typeof pattern === "string").filter((pattern) => !pattern.startsWith("!")).map(normalizePath);
	...
	])).filter((pattern) => typeof pattern === "string").map(normalizePath);
