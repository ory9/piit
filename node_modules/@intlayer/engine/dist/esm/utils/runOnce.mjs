...
const readSentinelState = async (sentinelFilePath) => {
	try {
		const [sentinelStats, raw] = await Promise.all([stat(sentinelFilePath), readFile(sentinelFilePath, "utf8")]);
...
const runOnce = async (sentinelFilePath, callback, options) => {
	...
	while (true) {
		...
		if (sentinelState) {
			...
			if (!forceRun && sentinelAge <= cacheTimeoutMs && sentinelState.version === packageJson.version) {
				await onIsCached?.();
				return;
			}
			await removeSentinel(sentinelFilePath);
