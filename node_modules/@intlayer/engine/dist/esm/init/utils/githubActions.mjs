import { getGitRootDir, listProjects } from "../../listProjects.mjs";
import { detectPackageManager } from "./packageManager.mjs";
import { join, relative, resolve, sep } from "node:path";
import { existsSync, readFileSync } from "node:fs";

//#region src/init/utils/githubActions.ts
/**
...
*/
const hasWorkspaceManifest = (repositoryRootDir) => {
	if (existsSync(join(repositoryRootDir, "pnpm-workspace.yaml"))) return true;
	try {
...
*/
const resolveGithubWorkflowsContext = async (rootDir, projectPackageManager) => {
	...
	try {
		const { projectsPath } = await listProjects({ baseDir: repositoryRootDir });
	...
	const hasRootPackageJson = existsSync(join(repositoryRootDir, "package.json"));
	if (!isNestedProject) return {
		workflowsRootDir: projectRootDir,
		packageManager: projectPackageManager,
		options: { useCiCommand: isWorkspaceRepository || repositoryProjectCount > 1 }
	};
	if (hasRootPackageJson) return {
	...
	return {
		workflowsRootDir: repositoryRootDir,
		packageManager: projectPackageManager,
		options: { workingDirectory: relative(repositoryRootDir, projectRootDir).split(sep).join("/") }
	};
};
/** Dependency lock file of each package manager, used for cache invalidation. */
const LOCK_FILE_BY_PACKAGE_MANAGER = {
	bun: "bun.lock",
	pnpm: "pnpm-lock.yaml",
	yarn: "yarn.lock",
...
*/
const getPackageManagerCIConfig = (packageManager, workingDirectory) => {
	const setupNodeStep = (cache) => {
		const cacheDependencyPath = cache && workingDirectory ? `\n          cache-dependency-path: ${workingDirectory}/${LOCK_FILE_BY_PACKAGE_MANAGER[cache]}` : "";
...
* repository root.
*/
const getWorkingDirectoryBlock = (workingDirectory) => workingDirectory ? `
    defaults:
      run:
...
*/
const getMonorepoCredentialsBlock = (useCiCommand) => useCiCommand ? `
      #
...
*/
const generateFillWorkflow = (packageManager, options) => {
	const { setupSteps, installCommand, execCommand } = getPackageManagerCIConfig(packageManager, options.workingDirectory);
	const intlayerCommand = options.useCiCommand ? `${execCommand} ci` : execCommand;
...
*/
const generateTestWorkflow = (packageManager, options) => {
	const { setupSteps, installCommand, execCommand } = getPackageManagerCIConfig(packageManager, options.workingDirectory);
	const intlayerCommand = options.useCiCommand ? `${execCommand} ci` : execCommand;
...
const GITHUB_FILL_WORKFLOW_PATH = ".github/workflows/intlayer-fill.yml";
const GITHUB_TEST_WORKFLOW_PATH = ".github/workflows/intlayer-test.yml";
/**
...
*/
const getGithubWorkflows = (packageManager, options = {}) => [{
...
//#endregion
export { GITHUB_FILL_WORKFLOW_PATH, GITHUB_TEST_WORKFLOW_PATH, getGithubWorkflows, resolveGithubWorkflowsContext };
//# sourceMappingURL=githubActions.mjs.map