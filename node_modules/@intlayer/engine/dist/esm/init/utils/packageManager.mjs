...
*/
const detectCompatI18nLibraries = (dependencies) => COMPAT_I18N_LIBRARIES.filter((library) => library.packages.some((packageName) => Boolean(dependencies[packageName]))).map((library) => library.label);
/**
...
* @param dependencies - Merged dependencies of the target project.
*/
const hasLintTooling = (dependencies) => Boolean(dependencies.eslint) || Boolean(dependencies.oxlint);
const detectMissingIntlayerPackages = (allDependencies, options = {}) => {
	...
	if (isInstalled("react-native") || isInstalled("expo")) addDevIfMissing("react-native-intlayer");
	...
	if (hasLintTooling(allDependencies)) addDevIfMissing("eslint-plugin-intlayer");
	...
	if (isInstalled("i18next") || isInstalled("@intlayer/i18next")) {
		...
		if (!isInstalled("react-i18next") && !isInstalled("@intlayer/react-i18next")) compatVitePluginConfig ??= {
...
const getInstalledPackageVersion = (rootDir, packageName) => {
	try {
		const manifestPath = join(rootDir, "node_modules", packageName, "package.json");
...
const detectOutdatedIntlayerPackages = (rootDir, dependencies, targetVersion) => {
	...
	return Object.keys(dependencies).filter(isIntlayerPackageName).filter((packageName) => {
		...
		const normalizedInstalled = normalizeVersion(installedVersion ?? void 0);
...
const upgradePackages = (rootDir, packages, packageManager, targetVersion, isDev = false) => {
	...
	const versionedPackages = packages.map((packageName) => `${packageName}@${targetVersion}`);
...
//#endregion
export { COMPAT_I18N_LIBRARIES, detectCompatI18nLibraries, detectMissingIntlayerPackages, detectOutdatedIntlayerPackages, detectPackageManager, getInstalledPackageVersion, hasLintTooling, installPackages, isIntlayerPackageName, normalizeVersion, upgradePackages };
