...
//#region src/init/utils/astImports.ts
...
*/
const isModuleScopeBinding = (ast, name) => ast.program.body.some((statement) => {
	const node = (statement.type === "ExportNamedDeclaration" || statement.type === "ExportDefaultDeclaration") && statement.declaration ? statement.declaration : statement;
	if (node.type === "ImportDeclaration") return node.specifiers.some((spec) => spec.local?.name === name);
	if (node.type === "VariableDeclaration") return node.declarations.some((decl) => decl.id?.type === "Identifier" && decl.id.name === name);
	if (node.type === "FunctionDeclaration" || node.type === "ClassDeclaration") return node.id?.name === name;
	return false;
});
/**
...
const ensureNamedImport = (ast, importName, source) => {
	...
	for (const statement of ast.program.body) if (statement.type === "ImportDeclaration" && statement.source.value === source) {
		if (!statement.specifiers.some((spec) => spec.type === "ImportSpecifier" && spec.imported?.name === importName)) statement.specifiers.push(b.importSpecifier(b.identifier(importName)));
		return;
	}
...
//#endregion
export { ensureNamedImport, firstInsertIndex, isModuleScopeBinding };
//# sourceMappingURL=astImports.mjs.map