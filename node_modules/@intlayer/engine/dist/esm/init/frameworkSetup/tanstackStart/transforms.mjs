...
import { ensureNamedImport, firstInsertIndex, isModuleScopeBinding } from "../../utils/astImports.mjs";
...
const ensureLocaleFromParams = (funcNode) => {
	...
	const statement = parseTsx(`const { locale = defaultLocale } = ${LOCALE_ROUTE_API_VARIABLE}.useParams();`).program.body[0];
...
* binding was reused.
*/
const ensureLocaleRouteApi = (ast, localeSegment) => {
	if (isModuleScopeBinding(ast, LOCALE_ROUTE_API_VARIABLE)) return false;
	const declaration = parseTsx(`const ${LOCALE_ROUTE_API_VARIABLE} = getRouteApi("/${localeSegment}");`).program.body[0];
...
*/
const separateRouteApiDeclaration = (printedCode) => printedCode.replace(new RegExp(`([^\\n])\\n(const ${LOCALE_ROUTE_API_VARIABLE} = getRouteApi\\()`), "$1\n\n$2");
...
*/
const wrapRootWithProvider = (code, localeSegment) => {
	...
	return {
		code: insertedRouteApi ? separateRouteApiDeclaration(printedCode) : printedCode,
		status: "wrapped"
	};
};

//#endregion
export { wrapRootWithProvider };
