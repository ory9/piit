import { rewriteRelativeImports } from "../nextAppRouter/restructure.mjs";
...
const restructureRoutesIntoLocale = async (rootDir, routesDir, localeSegment = LOCALE_SEGMENT) => {
	...
	for (const name of movedTopLevelNames) await rename(join(routesDirAbs, name), join(localeDirAbs, name));
	...
	for (const newAbs of movedFiles) {
		...
		if (rewritten !== code) await writeFile(newAbs, rewritten, "utf8");
	}
	return {
		status: "moved",
...
//#endregion
export { LOCALE_SEGMENT, PREFIX_ALL_LOCALE_SEGMENT, getLocaleSegment, isLocaleSegment, restructureRoutesIntoLocale, shouldKeepRouteAtRoot };
//# sourceMappingURL=restructure.mjs.map