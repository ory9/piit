...
*/
const normalizeCookieExpiry = (expires, maxAge) => {
	if (typeof maxAge === "number") return maxAge;
	if (expires === void 0) return void 0;
	if (expires instanceof Date || Object.prototype.toString.call(expires) === "[object Date]") return expires.toISOString();
	if (typeof expires === "string") return expires;
	return expires * SECONDS_PER_DAY;
};
const createCookieEntry = (options) => {
	const { name, path = "/", expires, maxAge, domain, secure, sameSite, httpOnly } = options ?? {};
	return {
