...
const createRules = (createElement, ctx, options, footnotes, refs, attrStringToMap, containsBlockSyntax, nonParagraphBlockSyntaxes) => {
	...
	return {
		...
		[require_markdown_constants.RuleType.orderedList]: generateListRule(1),
		[require_markdown_constants.RuleType.unorderedList]: generateListRule(2),
		...
		[require_markdown_constants.RuleType.text]: {
			...
			_parse(capture) {
				...
				return { text: text.indexOf("&") === -1 ? text : text.replace(require_markdown_constants.HTML_CHAR_CODE_R, (f, i) => {
