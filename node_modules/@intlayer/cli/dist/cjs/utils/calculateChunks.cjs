...
const chunkText = (text, maxCharsPerChunk = DEFAULT_MAX_CHARS_PER_CHUNK, overlapChars = DEFAULT_OVERLAP_CHARS) => {
	...
	const splittedText = (0, _intlayer_engine_utils.splitTextByLines)(text);
	const lines = [];
	let charStartAcc = 0;
	...
	for (let i = 1; i < splittedLines.length; i++) {
		...
		const overlapLineNb = (0, _intlayer_engine_utils.splitTextByLines)(overlapContent).length;
		...
		const newLineLength = (0, _intlayer_engine_utils.splitTextByLines)(newContent).length;
