...
const checkFileModifiedRange = (filePath, options) => {
	...
	if (shouldSkip) {
		...
		return {
			isSkipped: true,
			message: `Skipping file because it has been modified within the last ${require_utils_formatTimeDiff.formatTimeDiff(relativeTime)} - ${filePath}`
		};
	}
	return {
		isSkipped: false,
