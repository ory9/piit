...
*/
const RESERVED_CONTENT_FIELD_NAMES = /* @__PURE__ */ new Set(["nodeType"]);
...
const generateShortFieldName = (index) => {
	if (!Number.isInteger(index) || index < 0) throw new RangeError(`generateShortFieldName expects a non-negative integer index, received: ${index}`);
...
*/
const getNestedRenameEntryAtPath = (renameMap, fieldPath) => {
	let currentRenameMap = renameMap;
	let renameEntry;
	for (const fieldName of fieldPath) {
		renameEntry = currentRenameMap.get(fieldName);
		if (!renameEntry) return void 0;
		currentRenameMap = renameEntry.children;
	}
	return renameEntry;
};
/** Recursively converts a {@link NestedRenameMap} to its serialisable form. */
const serializeNestedRenameMap = (renameMap) => {
	const serialized = {};
	for (const [fieldName, renameEntry] of renameMap) serialized[fieldName] = {
...
* nothing to rename.
*/
const serializeFieldRenameMap = (pruneContext) => {
	const serialized = {};
	for (const [dictionaryKey, renameMap] of pruneContext.dictionaryKeyToFieldRenameMap) {
...
*/
const renameObjectPatternProperties = (babelTypes, objectPattern, scopePath, renameMap) => {
	for (const property of objectPattern.properties) {
		if (!babelTypes.isObjectProperty(property)) continue;
		if (property.computed && !babelTypes.isStringLiteral(property.key)) continue;
		...
		if (property.computed) property.key.value = renameEntry.shortName;
		else {
		...
		if (babelTypes.isObjectPattern(bindingTarget)) {
			renameObjectPatternProperties(babelTypes, bindingTarget, scopePath, renameEntry.children);
			continue;
		}
...
* `.onChange(cb)`.
*/
const renameContentConsumers = (babelTypes, rootPath, renameMap) => {
	const { finalPath, finalRenameMap } = walkRenameChain(babelTypes, rootPath, renameMap);
	walkObjectDestructuring(babelTypes, finalPath, finalRenameMap);
...
*/
const renameChainedRuntimeCall = (babelTypes, refPath, renameMap) => {
	if (renameMap.size === 0) return;
	const memberPath = refPath.parentPath;
	if (!memberPath) return;
	const memberNode = memberPath.node;
	if (!babelTypes.isMemberExpression(memberNode) && !babelTypes.isOptionalMemberExpression(memberNode) || memberNode.object !== refPath.node) return;
	const propertyNode = memberNode.property;
	const methodName = memberNode.computed ? babelTypes.isStringLiteral(propertyNode) ? propertyNode.value : void 0 : babelTypes.isIdentifier(propertyNode) ? propertyNode.name : void 0;
	if (!methodName || !require_babel_plugin_intlayer_usage_analyzer.CHAINABLE_RUNTIME_METHOD_NAMES.has(methodName)) return;
	const callPath = memberPath.parentPath;
	if (!callPath) return;
	if (!callPath.isCallExpression() && !callPath.isOptionalCallExpression()) return;
	if (callPath.node.callee !== memberNode) return;
	...
	for (const argumentPath of argumentPaths) {
		...
		if (firstParameterPath.isObjectPattern()) {
			renameObjectPatternProperties(babelTypes, firstParameterPath.node, argumentPath, renameMap);
			continue;
		}
		...
		for (const parameterRefPath of parameterBinding.referencePaths) renameContentConsumers(babelTypes, parameterRefPath, renameMap);
	}
	renameContentConsumers(babelTypes, callPath, renameMap);
	const declaratorNode = callPath.parent;
	if (babelTypes.isVariableDeclarator(declaratorNode) && babelTypes.isIdentifier(declaratorNode.id) && declaratorNode.init === callPath.node) {
		const resultBinding = callPath.scope.getBinding(declaratorNode.id.name);
		for (const resultRefPath of resultBinding?.referencePaths ?? []) renameContentConsumers(babelTypes, resultRefPath, renameMap);
	}
};
/**
...
*/
const collectIntlayerCallerLocalNames = (babelTypes, programNode) => {
...
*/
const renameIntlayerFieldAccesses = (babelTypes, programPath, pruneContext) => {
	if (pruneContext.dictionaryKeyToFieldRenameMap.size === 0) return;
	const intlayerCallerLocalNameMap = collectIntlayerCallerLocalNames(babelTypes, programPath.node);
	...
	programPath.traverse({ CallExpression: (callExpressionPath) => {
		...
		else if (babelTypes.isTemplateLiteral(firstArgument) && firstArgument.expressions.length === 0 && firstArgument.quasis.length === 1 && firstArgument.quasis[0]) dictionaryKey = firstArgument.quasis[0].value.cooked ?? firstArgument.quasis[0].value.raw;
		...
		if (babelTypes.isVariableDeclarator(parentNode) && babelTypes.isIdentifier(parentNode.id)) {
			const variableBinding = callExpressionPath.scope.getBinding(parentNode.id.name);
			if (!variableBinding) return;
			for (const variableReferencePath of variableBinding.referencePaths) {
				renameContentConsumers(babelTypes, variableReferencePath, fieldRenameMap);
				...
				if ((babelTypes.isCallExpression(refParent) || babelTypes.isOptionalCallExpression(refParent)) && refParent.callee === variableReferencePath.node) {
					...
					if (callPath) renameContentConsumers(babelTypes, callPath, fieldRenameMap);
				}
			}
		}
	} });
...
exports.getNestedRenameEntryAtPath = getNestedRenameEntryAtPath;
exports.makeFieldRenameBabelPlugin = makeFieldRenameBabelPlugin;
exports.renameIntlayerFieldAccesses = renameIntlayerFieldAccesses;
exports.serializeFieldRenameMap = serializeFieldRenameMap;
