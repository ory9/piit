...
*/
const COMPARISON_OPERATORS = /* @__PURE__ */ new Set([
	"===",
	"!==",
	"==",
	"!="
...
*/
const TECHNICAL_KEYS = /* @__PURE__ */ new Set([
	"icon",
	"className",
	"class",
	"id",
...
const NON_TRANSLATABLE_CALL_METHODS = /* @__PURE__ */ new Set([
	"includes",
	"startsWith",
	"endsWith",
	"indexOf",
...
*/
const getEnclosingMethodCallInfo = (path) => {
	const parent = path.parentPath;
	if (!parent.isCallExpression() && !parent.isOptionalCallExpression()) return;
	const callNode = parent.node;
	if (!callNode.arguments.includes(path.node)) return void 0;
	const callee = callNode.callee;
	if (!_babel_types.isMemberExpression(callee) && !_babel_types.isOptionalMemberExpression(callee)) return;
	return {
		methodName: _babel_types.isIdentifier(callee.property) ? callee.property.name : void 0,
		isConsoleCall: _babel_types.isIdentifier(callee.object) && callee.object.name === "console"
	};
};
/**
...
const handleJsxInsertionBabel = (path, fileCode, existingKeys, getComponentKeyForPath, extractedContent, replacements, handledNodes) => {
	...
	} else if (_babel_types.isJSXExpressionContainer(child)) {
		if (_babel_types.isJSXEmptyExpression(child.expression)) parts.push({
			type: "text",
			value: ""
		});
		else {
	...
	if ((0, _intlayer_config_extract.shouldExtract)(cleanString)) {
...
const extractBabelContentForComponents = (ast, fileCode, existingKeys, defaultKey = "default", configuration, filePath, unmergedDictionaries = {}) => {
	...
	traverse(ast, {
		...
		JSXText(path) {
			...
			if ((0, _intlayer_config_extract.shouldExtract)(text)) {
				const componentKey = getComponentKeyForPath(path);
		...
		JSXAttribute(path) {
			...
			if (typeof name !== "string" || !_intlayer_config_extract.ATTRIBUTES_TO_EXTRACT.includes(name)) return;
			const value = path.node.value;
			if (_babel_types.isStringLiteral(value) && (0, _intlayer_config_extract.shouldExtract)(value.value)) {
				const componentKey = getComponentKeyForPath(path);
		...
		StringLiteral(path) {
			if (handledNodes.has(path.node)) return;
			const text = path.node.value;
			if (!(0, _intlayer_config_extract.shouldExtract)(text)) return;
			const parent = path.parentPath;
		...
		TemplateLiteral(path) {
			...
			const cleanString = combinedString.replace(/\s+/g, " ").trim();
			if (!(0, _intlayer_config_extract.shouldExtract)(cleanString)) return;
			const componentKey = getComponentKeyForPath(path);
			const key = require_extractContent_utils_getOrGenerateKey.getOrGenerateKey(cleanString, componentKey, existingKeys, extractedContent);
