...
import { getRewritableCallers } from "@intlayer/config/callers";
...
const computeImport = (fromFile, dictionariesDir, dynamicDictionariesDir, fetchDictionariesDir, key, importMode, hasNestedDictionaries = false) => {
	let dictionaryPath = hasNestedDictionaries ? join(dictionariesDir, NESTED_DICTIONARIES_SUBDIR, `${key}.mjs`) : join(dictionariesDir, `${key}.json`);
	if (importMode === "fetch") dictionaryPath = join(fetchDictionariesDir, `${key}.mjs`);
	if (importMode === "dynamic") dictionaryPath = join(dynamicDictionariesDir, `${key}.mjs`);
	...
	return {
		...
		pre() {
			...
			if (this.opts.filesList && filename) {
				if (!getNormalizedFilesListSet(this.opts.filesList).has(filename)) {
					this._isIncluded = false;
					return;
				}
			}
		...
		visitor: { Program: {
			...
			*/
			exit(programPath, state) {
				...
				const rewritableCompatCallers = getRewritableCallers(state.opts.compatCallers ?? []);
