...
const getMultilingualUrls = (url, options = {}) => {
	const resolved = require_localization_getPrefix.resolveRoutingConfig(options);
	return Object.fromEntries((resolved.locales ?? []).map((locale) => [locale, require_localization_getLocalizedUrl.getLocalizedUrl(url, locale, resolved)]));
};
