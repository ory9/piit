...
var CrossFrameMessenger = class {
	...
	*/
	...
	_handleMessage(event) {
		...
		if (!(!allowedOrigins || allowedOrigins.length === 0 || allowedOrigins.includes("*") || allowedOrigins.filter((url) => Boolean(url) && url !== "").some((url) => require_compareUrls.compareUrls(url, event.origin)))) {
