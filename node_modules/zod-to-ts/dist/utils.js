...
export function handleUnrepresentable(unrepresentable, unrepresentableType) {
...
export function createAuxiliaryTypeStore() {
    let id = 0;
    return {
        nextId() {
            const currentId = id;
...
export function literalValueToLiteralType(literalValue) {
    switch (typeof literalValue) {
        case 'string':
            return f.createLiteralTypeNode(f.createStringLiteral(literalValue));
        case 'number':
            if (literalValue < 0) {
                return f.createLiteralTypeNode(f.createPrefixUnaryExpression(ts.SyntaxKind.MinusToken, f.createNumericLiteral(Math.abs(literalValue))));
            }
            return f.createLiteralTypeNode(f.createNumericLiteral(literalValue));
        case 'bigint':
            return f.createLiteralTypeNode(f.createBigIntLiteral(`${literalValue.toString()}n`));
        case 'boolean':
