...
import { addJsDocComment, createTypeReferenceFromString, getIdentifierOrStringLiteral, } from "./ast-helpers.js";
import { resolveOptions, } from "./types.js";
import { handleUnrepresentable, literalValueToLiteralType } from "./utils.js";
...
function withAuxiliaryType(schema, getInner, options) {
    ...
    if (auxiliaryTypeDefinition.used) {
        auxiliaryTypeDefinition.node = f.createTypeAliasDeclaration(undefined, auxiliaryTypeDefinition.identifier, undefined, node);
        return f.createTypeReferenceNode(auxiliaryTypeDefinition.identifier);
    }
    internalDefinitionsMap.delete(schema);
    return node;
}
export function zodToTs(zod, options) {
...
function zodToTsNode(schema, options) {
    ...
    switch (def.type) {
        ...
        case 'literal': {
            ...
            const members = def.values.map((value) => literalValueToLiteralType(value));
            if (members.length === 1) {
                return members[0];
            }
        ...
        case 'object': {
            return withAuxiliaryType(schema, () => {
                ...
                const members = properties.map(([key, memberZodSchema]) => {
                    ...
                    const propertySignature = f.createPropertySignature(undefined, getIdentifierOrStringLiteral(key), isOptional
                ...
                if (def.catchall) {
                    ...
                    members.push(f.createIndexSignature(undefined, [
                        f.createParameterDeclaration(undefined, undefined, f.createIdentifier('x'), undefined, f.createKeywordTypeNode(ts.SyntaxKind.StringKeyword)),
                    ], catchallType));
                }
        ...
        case 'enum': {
            // z.enum(['a', 'b', 'c']) -> 'a' | 'b' | 'c
            const members = Object.values(def.entries).map((value) => literalValueToLiteralType(value));
            return f.createUnionTypeNode(members);
        }
        case 'union': {
        ...
        case 'record': {
            ...
            const node = f.createTypeLiteralNode([
                f.createIndexSignature(undefined, [
                    f.createParameterDeclaration(undefined, undefined, f.createIdentifier('key'), undefined, keyType),
                ], valueType),
            ]);
            return node;
        }
        ...
        case 'function': {
            ...
            const parameterTypes = argumentSchemas.map((argument, index) => {
                ...
                return f.createParameterDeclaration(undefined, undefined, f.createIdentifier(`args_${index}`), undefined, parameterType);
            });
            const returnType = zodToTsNode(def.output, options);
            const node = f.createFunctionTypeNode(undefined, parameterTypes, returnType);
            return node;
        }
        case 'prefault':
        case 'default': {
        ...
        case 'readonly': {
            ...
            if (ts.isArrayTypeNode(innerType) || ts.isTupleTypeNode(innerType)) {
                return f.createTypeOperatorNode(ts.SyntaxKind.ReadonlyKeyword, innerType);
            }
            if (ts.isTypeLiteralNode(innerType)) {
                const members = innerType.members.map((member) => {
                    if (ts.isPropertySignature(member)) {
                        return f.updatePropertySignature(member, [
                            ...(member.modifiers ?? []),
            ...
            if (ts.isTypeReferenceNode(innerType) &&
                ...
                if (identifier === 'Set')
                    return f.createTypeReferenceNode(f.createIdentifier('ReadonlySet'), innerType.typeArguments);
                if (identifier === 'Map')
                    return f.createTypeReferenceNode(f.createIdentifier('ReadonlyMap'), innerType.typeArguments);
            }
        ...
        case 'template_literal': {
            ...
            for (const node of partNodes) {
                ...
                if (currentTypeSpanNode) {
                    const templateSpan = f.createTemplateLiteralTypeSpan(currentTypeSpanNode, f.createTemplateMiddle(currentMiddle));
            ...
            if (templateHead && currentTypeSpanNode) {
                const templateSpan = f.createTemplateLiteralTypeSpan(currentTypeSpanNode, f.createTemplateTail(currentMiddle));
                templateSpans.push(templateSpan);
            }
            else {
                return f.createLiteralTypeNode(f.createNoSubstitutionTemplateLiteral(currentMiddle));
            }
